...
export function init(chart: NapchartType) {
  ...
  function addListeners(element, eventNames, listener) {
    eventNames.split(' ').forEach((eventName) => {
      ...
      chart.destroyers.push(() => element.removeEventListener(eventName, listener))
    })
  }
...
function move(e, chart: NapchartType) {
  ...
  if (chart.listeningForPenMove) {
    ...
    var info = XYtoInfo(chart, coordinates.x, coordinates.y)
    ...
    var scratchDistance = duration(initialPenLocation, currentLocation)
...
function up(e, chart) {
  ...
  if (Object.keys(chart.activeElement).length > 0) {
    ...
    if (duration(element.start, element.end) == 0) {
      chart.deleteElement(id)
    } else {
      chart.history.add(chart, 'Move element')
    }
  }
...
function update(chart) {
  ...
  function move(hit, info) {
    ...
    if (hit.type == 'start' || hit.type == 'end') {
      ...
      if (collision) {
        if (hit.type == 'start') {
          if (duration(minutes, collision.end) > 720) {
            minutes = end
          } else {
            minutes = collision.end
          }
        } else {
          if (duration(collision.start, minutes) > 720) {
            minutes = start
          } else {
      ...
      // move the bastard
      originElement.duration = duration(originElement.start, originElement.end)
      ...
      if (collision) {
        // start = collision.end
        // end = start + duration
        var elementsInLane = chart.data.elements
          .filter((element) => {
            return (
              element.lane == collision.lane && // same lane
              element.id != originElement.id
            ) // exclude self
          })
          .sort(function (a, b) {
        ...
        var middlePoint = limit(info.minutes - positionInElement + originElement.duration / 2)
        // find gaps
        var gapsInLane = elementsInLane
          .map((element, i) => {
          ...
          .map((element) => {
            // calculate duration
            element.space = duration(element.start, element.end)
            return element
          })
          .filter((element) => {
        ...
        var gapElement = gapsInLane.reduce(
          (bestGap, currentGap) => {
            var d2start = minutesDistance(currentGap.start, middlePoint)
            var d2end = minutesDistance(currentGap.end, middlePoint)
            if (d2start < d2end) {
        ...
        if (gapElement.startOrEnd == 'start') {
          ...
          end = start + duration(originElement.start, originElement.end)
        } else {
          end = gapElement.end
          start = end - duration(originElement.start, originElement.end)
        }
      }

      // changes (if it has changed)
      if (originElement.start != start || originElement.end != end) {
        changes.start = limit(start)
        changes.end = limit(end)
      }
...
function isCollision(chart, start, end, lane, id = null) {
  ...
  var found = chart.data.elements.find((element) => {
    ...
    return (
      (isInside(element.start + 1, start, end) ||
        isInside(element.end - 1, start, end) ||
        isInside(start, element.start + 1, element.end - 1)) && // if element interferes AND...
  ...
  if (!collides) {
    ...
    var closestElement = chart.data.elements.reduce(
      (bestElement, thisElement) => {
        ...
        var distance = range(ownElement[stationaryStartOrEnd], thisElement[stationaryStartOrEnd], anticlockwise)
