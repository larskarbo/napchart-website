// Global chart helpers object for utility methods
// Some bloat here rn, but a lot of functions are important
...
helpers.arcToPath = (path, centerX, centerY, radius, startAngle, endAngle, anticlockwise) => {
  ...
  let start, end
  start = polarToCartesian(centerX, centerY, radius, startAngle)
  end = polarToCartesian(centerX, centerY, radius, endAngle)
  ...
  // path.A(radius, radius, 0, 0, anticlockwise ? 0 : 1, end)
  path += ` A ${radius} ${radius} 0 0 ${anticlockwise ? 0 : 1} ${end.x} ${end.y}`
  return path
...
helpers.minutesInDay = function (time) {
...
helpers.getProgressBetweenTwoAngles = function (pos, angleStart, angleEnd) {
  var result = helpers.angleDuration(angleStart, pos) / helpers.angleDuration(angleStart, angleEnd)
  if (isNaN(result)) {
    // I am doing this check because when dividing 0/0 it outputs NaN
    return 1
  } else {
...
helpers.isInsideAngle = function (point, start, end) {
  const { limitAngle } = helpers
  return helpers.isInside(limitAngle(point), limitAngle(start), limitAngle(end))
}

helpers.limitAngle = function (angle) {
  // limits values to between 0 and 2*Math.PI
  angle %= Math.PI * 2
  if (angle < 0) {
    angle += Math.PI * 2
  }
...
helpers.minutesToClock = function (chart, min) {
  var minutes = Math.floor(min)
  ...
  if (chart.config.ampm) {
    ...
    if (hours < 12) {
      return (hours == 0 ? 12 : (hours * 1)) + ':' + minutes + ' am'
    } else {
      return (hours == 12 ? 12 : (hours * 1 - 12)) + ':' + minutes + ' pm'
    }
  } else {
